...
<script type="text/javascript">
	...
	function checkname(){
		...
		if(name==null || name==""){
			namespan.css("color","red").html("用户名不能为空！");
		}else{
	...
	function checkpwd(){
		...
		if(password==null || password==""){
			pwdspan.css("color","red").html("密码不能为空！");
		}else{
			pwdspan.css("color","green").html("密码OK！");
		}
	}
</script>
...
<%
	...
	if(cks!=null){
		for(Cookie cookie:cks){
			if(cookie.getName().equals("unamecookie")){//获取cookie
				String adminname=cookie.getValue();    //获取cookie值
				request.setAttribute("adminname", adminname);//放入作用域 供 le 表达式提取
			}
		}
	}
...
<div class="login_div">
	...
	<form action="<%=path%>/afterlogin" method="post">
		<p style="padding: 30px 0px 10px; position: relative;">
			<span class="u_logo"></span>
			<input class="ipt" id="name" type="text" name="username" 
			placeholder="请输入用户名" value="${adminname}" 
			onblur="checkname()"
			required="required"> 
		</p>
	<span id="namespan"></span>
		<p style="position: relative;">
			...
			<input class="ipt" id="password" name="password"
			 type="password" placeholder="请输入密码" value="admin" 
			 onblur="checkpwd()" required="required">   
		</p>
